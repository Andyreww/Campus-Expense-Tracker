...
async function init() {
    ...
    renderAll();
    // Use requestAnimationFrame to ensure layout is painted before calculating indicator position
    ...
    itemsToDisplay.slice(0, 100).forEach(item => {
        ...
        row.innerHTML = `
            <div class="item-row-details">
                ...
                <div class="item-row-price-container">
                    <span class="item-row-price">$${item.price.toFixed(2)}</span>
                    ${item.onSale ? `<span class="item-row-original-price">$${item.originalPrice.toFixed(2)}</span>` : ''}
                </div>
            </div>
...
function renderCart() {
    ...
    cart.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <button class="add-to-subs-btn" data-id="${item.id}" title="Add to weekly subscriptions">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 2.7a2.7 2.7 0 0 1 3.8 0L22 4.9a2.7 2.7 0 0 1 0 3.8L13.4 17a2 2 0 0 1-1.4.6H8.5a1 1 0 0 1-1-1v-3.5a2 2 0 0 1 .6-1.4z"></path><path d="M8.5 2.5a1 1 0 0 0-1 1v2.5"></path><path d="M3.5 8a1 1 0 0 0 1 1h2.5"></path><path d="M12.5 21.5a1 1 0 0 0 1-1v-2.5"></path><path d="M18 12.5a1 1 0 0 0-1-1h-2.5"></path></svg>
            </button>
            <button class="remove-item-btn" data-id="${item.id}" title="Remove from cart">&times;</button>
        `;
    ...
    if (pastSubs.length === 0) {
        ...
        pastSubs.forEach(sub => {
            ...
            div.innerHTML = `
                ...
                <div class="cart-item-details">
                    <div class="cart-item-name">${sub.item.name}</div>
                    <div class="sub-duration">Active for ${weeksActive} week(s)</div>
                </div>
                <div class="cart-item-price">$${sub.item.price.toFixed(2)}</div>
            `;
...
async function logPurchase() {
    ...
    try {
        ...
        if (lastLogDate) {
            ...
            // If diffDays is 0, it's the same day, do nothing.
        } else {
            // First ever log
            currentStreak = 1;
        }
        ...
        // Update balances and streak info in one go
...
function getEmojiForItem(name, category = '') {
    ...
    // First, check category for a generic emoji
    ...
    if (lowerCat.includes('produce')) return '🍎';
    if (lowerCat.includes('pantry')) return '🥫';
    if (lowerCat.includes('household')) return '🧻';
    if (lowerCat.includes('dips & spreads')) return '🫙';

    // Then, check for more specific keywords in the name
    const keywords = {
        '⚡️': ['monster', 'red bull', 'energy drink', 'bang', 'celsius', 'prime energy'],
        '☕️': ['coffee', 'starbucks', 'latte', 'cold brew', 'espresso', 'frappuccino'],
        '💧': ['water', 'essentia', 'fiji', 'smartwater', 'poland spring', 'liquid death', 'bubbly', 'sparkling water'],
        '🥤': ['soda', 'coke', 'pepsi', 'sprite', 'dr pepper', 'fanta', 'root beer', 'mountain dew'],
        '🧃': ['juice', 'snapple', 'capri sun', 'oj', 'orange juice', 'apple juice', 'fruit punch', 'tropicana'],
        '🥛': ['milk', 'chocolate milk', 'oat milk', 'almond milk', 'soy milk', 'whole milk', '2% milk', 'fairlife'],
        '🧋': ['boba', 'bubble tea', 'milk tea', 'tapioca', 'taro tea'],
        '🥤': ['protein shake', 'protein drink', 'ensure', 'muscle milk'],
        
        '🍔': ['burger', 'cheeseburger', 'hamburger', 'mcdonalds', 'whopper'],
        '🍟': ['fries', 'french fries', 'curly fries', 'waffle fries'],
        '🌭': ['hot dog', 'frank', 'wiener'],
        '🌮': ['taco'],
        '🌯': ['burrito', 'chipotle', 'quesarito'],
        '🍕': ['pizza', 'pepperoni', 'cheese slice'],
        '🥪': ['sandwich', 'sub', 'wrap', 'panini', 'hoagie', 'blt'],
        '🍗': ['chicken', 'fried chicken', 'nuggets', 'wings', 'tenders'],
        '🥩': ['jerky', 'steak', 'beef', 'ribs'],
        '🍝': ['pasta', 'spaghetti', 'lasagna', 'mac n cheese'],
        '🍜': ['ramen', 'noodle', 'pho', 'udon', 'instant noodles'],
        '🍚': ['rice', 'white rice', 'fried rice', 'brown rice'],
        '🍱': ['bento', 'rice box'],
        '🍛': ['curry', 'japanese curry', 'indian curry'],
        '🥗': ['salad', 'greens', 'caesar', 'acai bowl', 'fruit bowl'],
        '🥣': ['cereal', 'oatmeal', 'granola', 'overnight oats'],
        '🥞': ['pancakes'],
        '🧇': ['waffles'],

        '🍩': ['donut', 'doughnut', 'krispy kreme', 'dunkin'],
        '🍪': ['cookie', 'oreo', 'chips ahoy', 'biscoff'],
        '🍫': ['chocolate', 'kitkat', 'hershey', 'snickers', 'twix'],
        '🍬': ['candy', 'skittles', 'gummy', 'sour patch', 'jolly rancher', 'licorice', 'starburst'],
        '🍰': ['cake', 'cheesecake', 'birthday cake'],
        '🧁': ['cupcake'],
        '🍦': ['ice cream', 'sundae', 'soft serve'],
        '🍨': ['gelato', 'sorbet'],
        '🍧': ['shaved ice', 'snow cone', 'halo halo'],
        '🍯': ['honey'],

        '🥜': ['peanut', 'nuts', 'trail mix', 'peanut butter'],
        '🍞': ['bread', 'toast'],
        '🥐': ['croissant', 'pastry', 'danish'],
        '🥯': ['bagel'],
        '🧈': ['butter', 'margarine'],

        '🍣': ['sushi', 'nigiri', 'sashimi', 'california roll'],
        '🍇': ['grape', 'grapes'],
        ...
        '🥕': ['carrot'],
        '🥔': ['potato', 'mashed potato', 'tater tots', 'baked potato'],
        '🌽': ['corn', 'elote'],
        ...
        '🥖': ['baguette'],
        '🍿': ['popcorn', 'movie snack'],
        '🧀': ['cheese', 'cheez-it', 'string cheese'],

        '🍫💪': ['protein bar', 'clif bar', 'quest bar', 'one bar'],
        '🥡': ['takeout', 'to-go', 'chinese food', 'leftovers'],
        '🧊': ['ice', 'ice cube', 'frozen'],
        '🫖': ['tea', 'green tea', 'black tea', 'chai', 'herbal tea'],
        };
...
function setupEventListeners() {
    ...
    cartItemsContainer.addEventListener('click', e => {
        ...
        if (removeBtn) {
            removeItemFromCart(Number(removeBtn.dataset.id));
        }
        if (subsBtn) {
    ...
    activeSubsList.addEventListener('click', e => {
        ...
        if (endBtn) {
            endSubscription(endBtn.dataset.id);
        }
    });

    modalConfirmBtn.addEventListener('click', () => {
        if (itemToSubscribe) {
            addSubscription(itemToSubscribe);
        }
    ...
    // Re-calculate slider on window resize using a debounce to prevent excessive calls
    let resizeTimer;
